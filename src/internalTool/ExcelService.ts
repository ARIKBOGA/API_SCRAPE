...
export async function processExcel(queryFilePath: string, keywordList: string[], originalFilename: string): Promise<string> {
    try {
        ...
        const newFilename = `${baseName}_Found_YV_Codes_${fileExtension}`;
...
function getQuerySet(jsonData: any[], keywordList: string[]): Set<string> {
    ...
    //fs.writeFile(path.resolve(__dirname, `../output/ALL/jsons/OE/KALE_BALATA.json`), JSON.stringify(jsonData, null, 2));
...
function main() {
    ...
    processExcel(inputFilepath, keywordList, ofn);
}
