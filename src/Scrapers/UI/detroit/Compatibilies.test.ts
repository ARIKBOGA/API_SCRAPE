...
async function scrapeAPI(
  ...
  for (const product of detroitProducts) {
    console.log(
      `🔍 Processing DETROIT_NO: ${product.DETROIT_NO}, YV_NO: ${product.YV_NO}`
    );
    const response = await apiContext.get(product.URL);

    if (!response.ok()) {
      console.error(
        `🚨 HATA: İstek başarısız oldu! Durum Kodu: ${response.status()}`
      );
      return [];
    }
        ...
        results.push({
          ...
          'Brand ID':
            ...
            markaNameToIdMap.get(
              brandAliases.get(
                thList.eq(1).text().trim().toUpperCase()
              ) as string
            ) ||
            null,
          ...
          From:
            parseCompatibilityYears(thList.eq(0).text().trim()).startYear || '',
  ...
  fs.writeFileSync(
    path.resolve(
      __dirname,
      `../../../output/${PRODUCT_TYPE}/jsons/Vehicle-Compatibility/Detroit_compatibilities.json`
    ),
...
test('API Request: Scrape compatibility table data with TH selector', async () => {
  ...
  const outputFilePath = path.resolve(
    __dirname,
    `../../../output/${PRODUCT_TYPE}/excels/Vehicle-Compatibility/Detroit-compatibilities.xlsx`
  );
