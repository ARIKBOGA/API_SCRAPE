...
export function writeOE_NumbersToExcel(filePrefix: string) {

    const INPUT_FILE_PATH = path.resolve(__dirname, `../output/jsons/${filePrefix}_Results.json`);
    const OUTPUT_DIR = path.resolve(__dirname, `../output/excels`);

    const joined: ResultRow[] = [];
    const single: ResultRow[] = [];
    const scrapedData: Result[] = JSON.parse(fs.readFileSync(INPUT_FILE_PATH, 'utf-8'));
    ...
    xlsx.writeFile(joinedWorkbook, `${OUTPUT_DIR}/${filePrefix}_Rockauto_Raybestos_OE_Numbers_Joined.xlsx`);
    xlsx.writeFile(singleWorkbook, `${OUTPUT_DIR}/${filePrefix}_Rockauto_Raybestos_OE_Numbers_Single.xlsx`);
}

writeOE_NumbersToExcel("November");