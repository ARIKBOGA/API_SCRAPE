...
import {
  ...
  ScrapedVehicleCompatibility_ExcelRow,
} from '../../utils/Types';
 ...
 */
export async function scraped_Compatibilities_JsonToExcel(
  results: ProductCompatibilityResult[],
  start: number,
  endCalc: number,
): Promise<void> {
  const OUTPUT_FILEPATH = PathRepo.output(
    `${PRODUCT_TYPE}/excels/Vehicle-Compatibility/Vehicle-Compatibility_${FILTER_BRAND}_${start}_${endCalc}.xlsx`,
  );
  ...
  try {
    ...
    results.forEach((item) => {
      ...
      item.compatibleVehicles.forEach((vehicle: OutputManufacturer) => {
        ...
        const marka_id =
          markaNameToIdMap.get(vehicle.manufacturer.trim().toUpperCase()) ||
          markaNameToIdMap.get(
            brandAliases.get(
              vehicle.manufacturer.trim().toUpperCase(),
            ) as string,
          ) ||
          null;
    ...
    await writeExcelSafe(OUTPUT_FILEPATH, {
      name: `${FILTER_BRAND}`,
      data: allRows,
    });
