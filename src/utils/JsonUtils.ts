...
function main() {
  ...
  .filter((file) => file.endsWith(".json") && file.includes("Pad_vehicle-compatibility_TEXTAR"));

  const mergedJson = jsonFiles
    .sort((a, b) => a.localeCompare(b))
    .reduce<any[]>((acc, file) => acc.concat(JSON.parse(fs.readFileSync(path.join(jsonsFolder, file), "utf8"))), []);
  const outputFilePath = path.join(jsonsFolder, "Merged_Pad_vehicle-compatibility_TEXTAR.json");
