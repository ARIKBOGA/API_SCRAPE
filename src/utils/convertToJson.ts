...
function convertToJSonFromExcel(filePath: string, sheetName: string) {
    ...
    fs.writeFile(path.join(__dirname, '../output/ALL/jsons/OE/ORJ_NO_KATALOG.json'), JSON.stringify(yv_oe_records, null, 2));
    fs.writeFile(path.join(__dirname, '../resources/data/catalogInfo/jsons/ORJ_NO_KATALOG.json'), JSON.stringify(yv_oe_records, null, 2));
}

convertToJSonFromExcel('../output/ALL/excels/OE/ORJ_NO_KATALOG.xlsx', 'Sheet1');